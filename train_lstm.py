...
def infer(source,
    ...
    with tf.Graph().as_default(), tf.Session(config=config) as sess:
        net = lstm_mdn.create_model(
            ...
            n_neurons=n_neurons, n_features=n_features,
            n_layers=n_layers, n_gaussians=n_gaussians,
            use_mdn=use_mdn)
    ...
    with tf.Graph().as_default(), tf.Session(config=config) as sess:
        ...
        res = (np.squeeze(prediction) * data_std) + data_mean
...
def test_euler():
    ...
    restore_name = 'lstm_mdn-euler-12'
...
def test_local_positions():
    ...
    restore_name = 'lstm_mdn-local-positions-29'
